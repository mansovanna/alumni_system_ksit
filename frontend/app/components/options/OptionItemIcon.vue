...
<template>
  ...
  >
    <button
      type="button"
      @click="toggle"
      class="w-full h-10 flex justify-between items-center gap-2 border border-slate-300 rounded-md px-2 py-2 bg-white focus:ring-2 focus:ring-primary/30 text-slate-500 text-sm"
    >
      <Transition name="fade-text" mode="out-in">
        <div class="flex justify-start items-center gap-2 text-primary">
          <HugeiconsIcon
            :icon="items.find((i) => i.name === props.active)?.icon ?? UserIcon"
            class="w-6 h-6 "
          />
          ...
          <div v-if="items" class="w-full flex flex-col gap-0.5">
            ...
            <div v-for="(item, index) in items" :key="item.id">
              <button
                ...
                :class="active == item.name ? 'bg-slate-200' : ''"
              >
                <div class="flex items-center gap-2">
                  <HugeiconsIcon :icon="item.icon" class="w-5 h-5" />
                  <span> {{ item.name }}</span>
                </div>
              </button>
            </div>
